...
def bigram(train_set, train_labels, dev_set, unigram_smoothing_parameter=0.035, bigram_smoothing_parameter=0.02,
    ...
    for i in range(len(train_set)):
        ...
        for j in range(len(train_set[i])):
            if train_labels[i] == 1:
                if train_set[i][j] not in dic_pos:
                    dic_pos[train_set[i][j]] = 1
                else:
                    dic_pos[train_set[i][j]] += 1
            else:
                if train_set[i][j] not in dic_neg:
                    dic_neg[train_set[i][j]] = 1
                else:
                    dic_neg[train_set[i][j]] += 1
    ...
    punk_pos_bi = math.log(bigram_smoothing_parameter / (positive_count_bi + bigram_smoothing_parameter * (pos_V_bi + 1)))
    punk_neg_bi = math.log(bigram_smoothing_parameter / (negative_count_bi + bigram_smoothing_parameter * (neg_V_bi + 1)))
    ...
    for l in dev_set:
        ...
        for word in l:
            if word in dic_pos:
                t = (dic_pos[word] + unigram_smoothing_parameter) / (positive_count + unigram_smoothing_parameter * (pos_V + 1))
                pos_total += math.log(t)
            else:
                pos_total += punk_pos

            if word in dic_neg:
                t = (dic_neg[word] + unigram_smoothing_parameter) / (negative_count + unigram_smoothing_parameter * (neg_V + 1))
        ...
        for i in range(len(l) - 1):
            st = l[i] + l[i + 1]
            if st in dic_pos_bi:
                t = (dic_pos_bi[st] + bigram_smoothing_parameter) / (positive_count_bi + bigram_smoothing_parameter * (pos_V_bi + 1))
                pos_total_bi += math.log(t)
            else:
                pos_total_bi += punk_pos_bi

            if st in dic_neg_bi:
                t = (dic_neg_bi[st] + bigram_smoothing_parameter) / (negative_count_bi + bigram_smoothing_parameter * (neg_V_bi + 1))
